...
export default function DashboardPage() {
  ...
  const handleSelectLead = async (sessionId) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/leads/${sessionId}`);
  ...
  const handleRouteLead = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/leads/${selectedLead.session_id}/route`, { method: 'POST' });
      if (res.ok) {
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/leads/${selectedLead.session_id}/summary`, { method: 'POST' });
      if (res.ok) {
